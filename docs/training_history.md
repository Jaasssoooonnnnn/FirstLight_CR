...
FirstLight CR 的训练主线围绕五个模型展开：**普通 IL、Active IL、General、Hog 2.6 specialist 1 和 Hog 2.6 specialist 2**。训练过程中，我们逐步解决开局消极、对手选择和速猪行为学习的问题，也经历过对被动 IL 胜率很高、实战表现却不理想的歪路。
...
| **General** | Stronger with cheap decks; expensive-deck play is more ordinary / 小费卡组玩得好一些，大费表现一般 | General-purpose RL model and the specialist's starting point / 通用强化模型，也是速猪专精的起点 |
| **[Hog 2.6 specialist 1](../checkpoints/2_6hog_expert/hog26-specialist1.pt)** | The more proactive Hog specialist / 更积极一些的速猪模型 | Specialist developed with demonstrations and Hog-specific shaping / 经示范监督和野猪专项奖励训练出的阶段成果 |
| **[Hog 2.6 specialist 2](../checkpoints/2_6hog_expert/hog26-specialist2.pt)** | The more passive Hog specialist, with stronger measured match results / 更被动一些，但模型对战成绩更强的速猪模型 | Final main specialist after removing Hog-specific shaping / 移除野猪专项奖励后得到的最终核心模型 |
...
“更积极”是两个速猪模型之间的相对特点。Hog 2.6 specialist 1 在 argmax 下也可能等待对手先动，并没有解决所有 matchup 的被动开局。

```mermaid
flowchart TD
    R[Human replay data / 真人回放] --> IL[IL]
    IL --> A[Active IL]
    IL -->|Initialize weights / 初始化权重| G[General / 通用]
    A -.->|Frozen opponent / 冻结对手| G
    G -->|Demonstrations + PPO + Hog shaping / 示范与专项奖励| Hog1[Hog 2.6 specialist 1 / 速猪]
    Hog1 -->|Remove Hog shaping; continue PPO / 去掉野猪专项奖励| Hog2[Hog 2.6 specialist 2 / 速猪]
```
...
Active IL 通过对手身份参与 General 的训练：**General 的初始化权重仍来自原始 IL**。General → Hog 2.6 specialist 1 之间包含数轮中间训练。调整训练配方时，多次只继承模型权重并重建优化器。
...
Active IL 的目标是在保留 IL 基础能力的同时，让模型更愿意开战。从原始 IL 出发，使用 **50% 自博弈＋50% 冻结 IL**，关闭强制开局 fallback，加入圣水溢出惩罚。先用较强惩罚修正行为，再减轻惩罚继续训练，最终选出第二阶段的 **Active IL**。
...
选出的 **General** 成为可用的通用强化模型，也成为后续专精的底座。实战中它的小费卡组玩得更好，大费表现一般。但 argmax 下空场等待的问题仍然存在，交战实力和主动开局依旧是两件事。

## 5. Teach Hog Cycle behavior · 训练更积极的 Hog 2.6 specialist 1
...
专精模型固定使用**觉醒加农炮、觉醒骷髅兵、精英火枪手、野猪骑士、冰人、冰精灵、火球和滚木**。最初从原始 IL 出发的尝试效果不佳，随后改从 General 初始化，主要固定对手组合为 **40% 原始 IL＋60% General**。
...
增加下猪奖励让模型更愿意下猪，却经常只是到后半场才多下。真实速猪回放为动作和开局提供了更直接的监督。经过数轮 PPO＋模仿短训，选出中间模型，关闭模仿监督继续 PPO；之后再调整首猪时间奖励，并按对手模型分别统计困难卡组，最终得到阶段成果 **Hog 2.6 specialist 1**。
...
Hog 2.6 specialist 1 是主线中更积极一些的速猪模型。训练中明确教过下猪和开局时机，但确定性评测仍暴露出它面对部分对手时等待先手的问题。

## 6. Let match outcomes lead · 得到更被动的 Hog 2.6 specialist 2
...
从 Hog 2.6 specialist 1 加载权重并重建优化器后，我们移除**全部野猪专项奖励和惩罚**，保留终局胜负、塔血塑形和圣水溢出惩罚，模仿监督继续关闭。对手仍为 **40% IL / 60% General**；对手卡组分布为 **30% 主流 / 40% 加权抽样 / 30% 困难卡组**。
...
较长的 PPO 训练得到最终核心模型 **Hog 2.6 specialist 2**。它的模型对战成绩提高了，打法却变得更被动。后来尝试修正溢费和误开国王塔，虽然改变了行为，但没有确立更强的替代版本，因此主线保留原始 Hog 2.6 specialist 2。
...
| Candidate / 候选 | Original IL / 原始 IL | Active IL | General |
| --- | ---: | ---: | ---: |
| General, mixed decks / 通用卡组 | 73.30% | 60.21% | — |
| Hog 2.6 specialist 1 / 固定速猪 | 84.74% | 84.08% | 79.74% |
| Hog 2.6 specialist 2 / 固定速猪 | 89.87% | 86.97% | 88.03% |
| Hog 2.6 specialist 2, mixed decks / 通用卡组 | 66.75% | 60.73% | 59.16% |
...
这些评测中，**只有原始 IL 在 15 秒仍未出牌时被强制执行一次首次动作**；Active IL 和 General 没有这项 fallback。复现时必须保留这一条件，发布版基础评测示例并不等同于历史评测协议。通用卡组与固定速猪是不同的任务分布，不能把各行直接连成一条胜率提升曲线。
...
Hog 2.6 specialist 2 仍保留了通用能力：完全排除己方含野猪的卡组后，对 General 胜率仍为 **59.45%**。但它并非全面升级，通用卡组对原始 IL 的胜率比历史 General **低 6.54 个百分点**。专精改变了能力分布，并不是每个方向都提高。
